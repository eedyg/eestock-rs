// =============================================================================
// T0 做T·主张段捕获 —— 多头趋势回踩入场 + MA趋势/斜率过滤 + ATR跟踪止损 + 收盘前强平
// 研发验证口径（2026-06-10~09-10，M5，ETF 费率 万0.5/无印花税/滑点2bp）：
//   样本内(06-10~08-10) 豆粕159985 年化+14.0%；样本外(08-10~09-10) 年化+48.9%（13 笔，pf 6.4）
//   中韩半导体513310 样本内+5.7%/样本外+1.8%（样本外仅 2 笔，统计意义弱，如实标注）
// 机制说明：close>MA240 且 MA240 上行（半日 lag）判定多头环境；
//   回踩 MA20（low 触线）且收回其上 → 主张段延续做多；ATR(14)×2 跟踪止损；
//   14:45 强制离场信号（次 bar 成交≈14:50），日内不留隔夜仓（T+0 做T 口径）。
// =============================================================================
const PARAMS_SCHEMA = [
  { key: "entry_mode",     type: "int",   default: 1,   min: 0,   max: 2,    description: "入场机制：0=开盘区间突破；1=多头回踩 MA；2=放量动量爆发" },
