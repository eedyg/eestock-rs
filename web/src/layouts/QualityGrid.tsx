...
export function QualityGrid(props: QualityGridProps) {
  return (
    <div data-region="quality" className="flex min-h-0 min-w-[1280px] flex-1 flex-col">

      {/* filter-bar：静态控件无三态；标的+日期范围+视图切换，变更即重查 */}
      <div data-region="filter-bar" className="flex h-12 shrink-0 items-center gap-2 border-b px-4">
        {/* <SymbolFilter/> <DateRangeFilter/> <ViewSwitch/> */}
      </div>

      {/* 主视图：分歧表（默认）或叠加图，二选一；min-h-0 允许收缩 → 大表在视口内滚动（页面高度受控） */}
      {props.view === 'table' ? (
        /* divergence-table：GET /api/quality/divergence?code=&from=&to=；默认偏差降序+汇总行；
            三态=骨架行/「该范围无比对数据」+补拉引导/错误条+重试 */
        <div data-region="divergence-table" className="flex min-h-0 flex-1 flex-col overflow-hidden">
          {/* <DivergenceTable onJumpToKline/> + <SummaryRow/> */}
        </div>
      ) : (
        /* overlay-chart：同 divergence 数据客户端渲染；三态随分歧表；缩放 */
        <div data-region="overlay-chart" className="flex min-h-0 flex-1 flex-col overflow-hidden">
      ...
      <div data-region="accuracy-cards" className="flex h-28 shrink-0 gap-2 border-b p-2">
