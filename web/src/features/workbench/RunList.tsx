...
export function RunList({
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-1 p-2 text-xs" data-testid="wb-run-list">
      <div className="flex shrink-0 items-center justify-between px-1 text-dim">
      ...
      {actionError && (
        <div className="shrink-0 rounded-lg border border-up/40 bg-up/10 px-2 py-1 text-up" role="alert" data-testid="wb-action-error">
          {actionError}
        </div>
      )}
      {/* 有界内部滚动容器：历史条数很多时行在容器内滚动，不撑开左列挤压配置区 */}
      <div className="flex min-h-0 flex-1 flex-col gap-1 overflow-y-auto" data-testid="wb-run-list-scroll">
      ...
      {hasMore && (
        <button
          type="button"
          className="mt-1 shrink-0 rounded-lg border border-line px-3 py-1 text-dim hover:text-txt disabled:opacity-40"
