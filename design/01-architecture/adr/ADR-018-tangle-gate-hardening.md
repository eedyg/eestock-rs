...
### 6.3 Docker agent 接入 MCP（2026-09-12 用户明确场景 + 架构师实测）

**用户场景**：将有 **docker agent** 访问平台 MCP 接口。⇒ §6.1 的"保持 0.0.0.0 绑定"同时满足
容器接入与局域网浏览器接入。

**实测证据（架构师，在容器网络命名空间内探测，非仅宿主自测）**：

| 探测点 | 目标 | 结果 |
|---|---|---|
| 容器 netns（`nsenter -t <data容器PID> -n`） | `http://172.17.0.1:8082/sse` | **http=200** |
| 容器 netns | `http://172.17.0.1:8081/healthz` | **http=200** |
| 宿主 | `http://172.17.0.1:8082/sse` / `http://192.168.50.100:8082/sse` | 均 200 |

**接入方式（按优先级）**：

1. **推荐：`network_mode: host`** —— 容器内直接用 `http://127.0.0.1:8082/sse`。
   同时规避已知的**客户端侧限制**：部分 MCP 客户端/gateway **拒绝非 loopback URL**（I-9 记录的现象，
   早前 `/tmp/relay.py` 中继即因此而生）。用 host 网络 + loopback URL 是最省事的正解。
2. **默认 bridge 网络**：容器内经 `http://172.17.0.1:8082/sse` 访问（实测 200）。
   若客户端强制 loopback，则需在容器内自建中继（不推荐，链路多一跳）。
3. 局域网其它主机：`http://192.168.50.100:8082/sse`（§6.1 形态）。

**注意**：MCP 为 **SSE 长连接**，容器侧客户端须支持 SSE（非 Streamable HTTP）；勿用一次性 HTTP 客户端。

**安全含义（在 §6.2 基础上补充）**：宿主 docker 网络内的任意容器（含第三方项目的容器）均可免认证
调用 `strategy_*`/`bt_*`/`sim_*`（单开关默认开）。当前 `sim_*` 为模拟实盘、无真实券商通道，风险可接受；
**若将来宿主上运行不受信任的容器，必须先落地 M-2（Token 鉴权）或拆分开关**（§6.2 可选加固）。
