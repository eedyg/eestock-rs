...
## E2E 测试栈（Playwright，2026-09-04定稿，用户拍板）

- **框架**：Playwright（真实浏览器多引擎）。定位：确保所有 web 功能在真实环境正确执行——单元/行为测试（Vitest，jsdom）无法验证 klinecharts canvas 真实渲染、WS 真实推送、真实点击流。
- **运行目标**：本地 `http://localhost:8081`（真实 app 容器 + 真实 DB，用户选 a）。断言必须**状态容忍**（交易日价格/时间在变——断言"有 bar/能推/能切"，不锁死具体数值）。
- **视觉回归基线**：从初始就建（用户选 b）。⚠️ 动态数据（价格/时间戳/图表内容）会产生分钟级噪声，基线必须**结构化**：用 Playwright `toHaveScreenshot` 对易变区域 `mask`（价格/时间/图内数据），只对**稳定结构**（导航/工具栏/布局/配色/区域坐标）做像素回归；重新基线化仅在有意的 UI 变更后人工执行。
- **用例分层**：全页冒烟（data-region 齐/路由/404/深链）+ 功能流（选股/切周期/分时/宫格/注册热生效/启停/告警确认/质量选日）+ WS 实时断言 + canvas 像素断言（非空抽样）。
- **写操作纪律**：测试用真实标的、测试后 SQL 清理（延续 tester 纪律）；不污染长期数据。
- **工程**：`web/e2e/`（Playwright TS 工程 + playwright.config.ts，baseURL 用 `E2E_BASE_URL` env 默认 `http://localhost:8081`）；`npm run e2e` / `e2e:update`（重基线）；失败自动 trace+截图存 `web/e2e/artifacts/`。

## 截图走查门槛（并入验收标准）
每页面交付前：E2E 全绿 + 全尺寸截图由用户过目（替代静态样机评审），用户点头页面才交付。
