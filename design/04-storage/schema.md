...
## 4.3.16 标的类型元数据与费率档案（ADR-019 / D11，0025；架构裁决 2026-09-12）

**上下文**：`strategy_test_run` / `bt_run_ensemble` 缺省印花税 0.05%（股票口径），而平台 44 只注册标的
**全部为 ETF/LOF（无个股）** → 缺省对 100% 标的是错的（实测：同一 ETF 同配置，默认口径 pnl 少 50.83 元、差 3.2%）。
根因是缺标的类型元数据 → 费率无法按类型推断（ADR-019 背景，D11 立项）。

**① `symbols.type`（D11-1）**：可空 `text` 枚举列。**NULL = 未知，刻意不设 NOT NULL 默认值**——架构裁决 A2：
任何具体类型默认值都会静默错判另一类标的（默认 `etf` 会把将来注册的个股按 ETF 计费；默认 `stock` 会把 ETF 按股票口径计费）。
CHECK 限定 ADR-019 D11-1 枚举（含 D11-6 为债券/货币 ETF/指数预留的 `bond_etf`/`money_etf`/`index`）。
未设 type 的标的在费率解析中走**第三级回退**（旧 ADR bt-1 默认 + 回显 `source="default"`），不借用他类型档案。

> **v1.1 修订（ADR-019 §6，2026-09-12 部署后回归）**：费率解析改为**按字段优先级**（R-2）——显式 `fee` 对象
> **出现的字段**优先，**缺失字段逐字段回退档案**，档案缺失再回退旧默认；`source` 取**最高优先级来源**（R-3）。
> `strategy_run.config.fee` 保持**扁平**（`fee_model_to_json`，R-1）；两段结构（`effective`/`profile`）**仅用于响应回显**，
> **不得混入 config**（旧行兼容：读取路径需容忍历史两段形态，不得崩）。

**② `fee_profiles`（D11-2）**：按 `type` 主键的费率档案。单位=百分数（`0.025`=万2.5、`0.00341`=0.0341‰），
与 `backtest::FeeModel` 同口径。口径（ADR-019 §1）：
- `etf` / `lof` = 场内基金二级市场买卖：印花税**不征**→0、过户费**免收**→0、经手费/证管费**按全佣口径已含于佣金 → 列 0（不得叠加）**；
- `stock` = A 股事实费率（印花税卖出 0.5‰、过户费 0.01‰、经手费 0.0341‰、证管费 0.02‰）。

`note`/`source` 逐行记录口径与来源（D11-2「每行须带口径说明」）。**本批引擎只消费 `commission_rate_pct`/`min_fee`/`stamp_duty_pct`**
（`backtest::FeeModel` 4 参数；滑点非费率事实、取 ADR bt-1 默认 2bp）；`exchange_fee_pct`/`regulatory_fee_pct`/`transfer_fee_pct`
为**事实/口径列，引擎未建模**（`bond_etf`/`money_etf`/`index` 保留位本批**不播种** —— D11-6；登记为 **D11-follow-up 债务**：
注册个股或需规费精度时须扩展 FeeModel，即经手费/证管费/过户费双边、印花税仅卖出侧）。
当前标的的债券/商品/跨境子类**统一记 `etf`**（子类仅记分类清单：`coder/report/147_d11_classification_review.md`，
架构师 2026-09-12 复核批准 **42 etf / 2 lof / 0 stock**）。
**不设 `symbols.type`→`fee_profiles.type` 外键**：保留位枚举值可以没有档案行，缺档案 → 回退旧默认（fail-soft，而非注册被拒）。

**③ 逐码回填（D11-4）**：44 只既有标的的 type 由**显式清单**回填（无启发式：代码段 + 名称关键词 + 人工复核，
依据逐条列于分类清单）。回填 `WHERE s.type IS NULL` → 重复执行安全，且不覆盖运营侧人工维护值。

**幂等与应用**：`ADD COLUMN IF NOT EXISTS` / 约束 DO 块判存在 / `CREATE TABLE IF NOT EXISTS` / 回填仅填 NULL /
播种 `ON CONFLICT (type) DO NOTHING`。应用方式 `psql -f migrations/0025_symbol_type_fee_profiles.sql`
（重复执行输出 NOTICE 属正常，安全）。**全新库注意**：initdb 阶段 `symbols` 为空 → 回填 0 行，
新标的的 type 须经 `POST /api/symbols`（可选 `type` 入参）或 `PATCH /api/symbols/{code}` 写入（见 00-web-api §8）。

``` {.sql file=migrations/0025_symbol_type_fee_profiles.sql}
...
**storage 模块 `crates/storage/src/fee_profile.rs`（非 tangle 手写，契约描述）**：
实现 `domain::ports::FeeProfileStore`（PgPool；fee_profiles + symbols.type，迁移 0025）。
- `for_symbol(code)`：`SELECT p.* FROM symbols s JOIN fee_profiles p ON p.type = s.type WHERE s.code = $1`；
