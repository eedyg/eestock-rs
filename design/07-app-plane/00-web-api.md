...
async fn merged_1m_branch_index_limit_performance() {
    ...
    // **复审条件（登记，机器可检索）**：将来若 ≥20 次连跑的**比值 max ≥0.15 则复审**
    //   （0.25/0.15 = 1.67× <2× 余量，余量已被吃掉过半）。依据：稳态最坏实测比值 0.075（诱导负载
    //   load1 1.6→9.6）时，阈值 0.25 仍留 **3.3×** 余量（0.25/0.075 = 3.33×）；一旦 max 逼近 0.15
    //   即说明该余量几近耗尽，届时再考虑 (i) 两侧样本增至 min-of-9 或 (ii) 「比值 + 宽松绝对上限」
    //   双判据——**当前数据不支持该复杂度**（登记与论证见 `coder/report/151_debt_cleanup_batch1.md`）。
    //
